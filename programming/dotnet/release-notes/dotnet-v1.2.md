...
- Added a new property `PageNumber`({{site.dotnet-structs}}dlr-result-v1.1.html#pagenumber) to `DLR_Result` to identify the page on which the result is located.
...
- Add a new API `AppendSettingsFromFile`({{site.dotnet-api-methods}}settings-v1.1.html#appendsettingsfromfile) to allow appending settings directly from a JSON file.
